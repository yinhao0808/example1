...
sgl_admm <- function(X,Y,rho, group, lambda1, lambda2, b0, iter=500, family = "gaussian", e.abs=1E-3,e.rel=1E-3){
  ...
  for (t in 2:iter){
    ...
    y.work = E%*%alpha[t-1,] +ww^(-1)*(Y_origin-P0)
...
sgl_admm_sample <- function(X,Y,rho, group, fold=1, lambda1, lambda2, b0, family = "gaussian", iter=500,e.abs=1E-3,e.rel=1E-3){
  ...
  groupID_sample <- split(1:(groupsize_sample), rep(1:numofgroups_sample, rep(groupsize_sample, numofgroups_sample)))
  ...
  for ( i in seq(length(groupID_sample))){
    ...
    invmat_group[[i]] <- diag(1/rho, ncol(X[tmp_id,])) - (1/rho)*t(X[tmp_id,])%*%solve(weight_adjust*diag(rho, nrow(X[tmp_id,])) + X[tmp_id,]%*%t(X[tmp_id,]))%*%X[tmp_id,]
  ...
  for (t in 2:iter){
    ...
    for ( i in seq(length(groupID_sample))){
      ...
      tmp_id <- groupID_sample[[i]]
      #invmat <- diag(1/rho, ncol(X[tmp_id,])) - (1/rho)*t(X[tmp_id,])%*%solve(groupsize*diag(rho, nrow(X[tmp_id,])) + X[tmp_id,]%*%t(X[tmp_id,]))%*%X[tmp_id,]  #solve(t(X[tmp_id,])%*%X[tmp_id,]+rho*diag(p))
      beta_i[[i]] <-  invmat_group[[i]]%*%(XtY_group[[i]]/(weight_adjust)+rho*(gamma[t-1,] - v[i,])) #invmat%*%(t(X[tmp_id,])%*%Y[tmp_id]/groupsize+rho*(gamma[t-1,] - v[i,]))
      end_time <- Sys.time()
      time2 <- c(time2, as.numeric(end_time - start_time))
    }
    time2 <- max(time2)
...
sgl_admm_par <- function(X, Y, rho, b0=b0, lambda1, lambda2, numofgroups, iter=50000,e.abs=1E-3,e.rel=1E-6){
  ...
  groupsize <- p/numofgroups
  groupID <- split(1:(groupsize*numofgroups), rep(1:numofgroups, rep(, numofgroups)))
  ...
  for (t in 2:iter){
    ...
    for (i in seq(numofgroups)){
      ...
      beta_i <- rbind(beta_i,  l2(beta_i_tmp, lambda2/rho))# 1/n
      end_time <- Sys.time()
      time2 <- c(time2, as.numeric(end_time - start_time))
    }
    beta[t,] <- beta_i
    ...
    gamma[t,] <- 1/(numofgroups+rho)*(rho*X_i_beta_i_mean_new + rho*v )
...
sgl_admm_par_v2 <- function(X, Y, rho, b0=b0, lambda1, lambda2, group, fold=1, family = "gaussian", iter=500,e.abs=1E-3,e.rel=1E-3){
  ...
  for (t in 2:iter){
    ...
    for (i in seq(fold)){
      ...
      for(jj in seq(group_fold)){
        ...
        M = X_temp[,j.index]%*%beta_temp[j.index] + gamma[t-1,] - X_i_beta_i_mean 
        den <- l2norm(beta_temp[j.index])
        if (den == 0){den=10^(-6)}
        g <- 1+(lambda2/rho)/den
        beta_i_j_tmp <- l1(1/g*t(X_temp[,j.index])%*%M,lambda1/(rho))
  ...
  for (t in 2:iter){
    ...
    start_time <- Sys.time()
    X_i_beta_i_mean_new <- 1/fold*X%*%beta[t,]
    gamma[t,] <- 1/(fold+rho)*(rho*X_i_beta_i_mean_new + rho + Y)
